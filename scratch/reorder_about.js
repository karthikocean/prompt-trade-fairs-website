...
// 2. Segment 4 replacement (Pan-India Network)
...
// 3. Segment 5 replacement (Exhibition Management)
...
const cleanOld2 = oldSection2.split('\n').map(l => l.trim()).join('\n');
const cleanContent = finalContent.split('\n').map(l => l.trim()).join('\n');

// Standardized replace helper
function replaceSegment(oldStr, newStr) {
    const linesOld = oldStr.split('\n').map(l => l.trim()).filter(Boolean);
    const linesNew = newStr.split('\n').map(l => l.trim()).filter(Boolean);
    ...
    if (startIdx !== -1) {
        // Replace from startIdx to startIdx + linesOld.length
